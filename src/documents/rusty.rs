...
Bir loop var bir de while ve for var.
...
Eğer array içerisinde for loop yapacak olsaydım:
for number in samplearr.iter() {
...
bir değer alacağız.. Bunu bilemeyiz. Bu tarz durumlar için Rust'ın ikinci bir string type'ı varmış:
...
İki kere yaparsak da bug (double free) olur. Bir allocate ile bir free işlemi birbirlerini tutmalı yani günün sonunda
